...
def load_json(source, default):
    """Загружает JSON из файла или строки"""
    try:
...
def task(temp_mf_json, heat_mf_json, rules_json, current_temp):
    ...
    for state in heat_mfs["уровень нагрева"]:
        for point in state["points"]:
            temperature = point[0]
            min_heat = min(min_heat, temperature)
            max_heat = max(max_heat, temperature)
...
if name == "main":
    # Задание значений по умолчанию для MF (членских функций) и правил
    default_temp_mf = {
        "температура": [
    ...
    try:
        # Вычисление оптимального уровня нагрева
